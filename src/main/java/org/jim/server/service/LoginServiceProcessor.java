...
public class LoginServiceProcessor implements LoginCmdProcessor {
	...
	@Autowired
	protected ImUserService imUserService;
	
	@Autowired
	protected IGroupService groupService;
	
	@Autowired
	protected LiveChatClientService liveChatClientService;
	...
	public void init() {
		...
		loginServiceProcessor.imUserService = this.imUserService; 
		loginServiceProcessor.groupService = this.groupService;
		loginServiceProcessor.liveChatClientService = this.liveChatClientService;
	...
	public User getUser(ImUser imUser) {
		String text = imUser.getUserName()+imUser.getPassword();
		String key = ImConst.authkey;
		String token = Md5.sign(text, key, HttpConst.CHARSET_NAME);
		User user = getUser(token, imUser);
		user.setId(imUser.getUserName());
		return user;
	}
	...
	public User getUser(String token, ImUser imUser) {
		User user = imUser.getUser();
		System.out.println("create group info /n");
		user.setGroups(getGroup(loginServiceProcessor.groupService.getAllGroup(imUser)));
	...
	private List<Group> getGroup(List<ChatGroup> allGroup) {
		List<Group> groups = new ArrayList<Group>();
		System.out.println(groups.size());
		for (ChatGroup cg: allGroup) {
			groups.add(new Group(cg.getId().toString(),cg.getGroupName()));
		}
		return groups;
	}
...
//	}
	...
	public LoginRespBody doLogin(LoginReqBody loginReqBody , ChannelContext channelContext) {
		...
		if (!StringUtils.isBlank(handshakeToken)) {
			...
			ImUser imUser = loginServiceProcessor.imUserService.getImUser(userName,password);
			if (imUser!=null) {
				user = this.getUser(imUser);
			}
			// 聊天系统上登入不了，则到客服系统判断是否可以登入
			if (imUser == null) {
				user = loginServiceProcessor.liveChatClientService.login(userName,password);
			}
		}
		if(user == null){
			loginRespBody = new LoginRespBody(Command.COMMAND_LOGIN_RESP,ImStatus.C10008);
